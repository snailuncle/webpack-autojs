function Maid(packageName) {
    ...
    this.clickMultiIndex=function(widgets,index){
        if (!widgets || widgets.length == 0)
        return;
        var rect =  widgets[index].bounds();
        this.click(rect.centerX(), rect.centerY());
    }
    //查出列表，点击其中一个
    this.clickMultiChildIndex=function(widgets,childSelector,index){
        if (!widgets || widgets.length == 0)
        return false;
        var wg =  widgets[index].findOne(childSelector);
        if(wg==null){
            return false;
        }
         var rect=   wg.bounds();
    ...
    this.clickTextStartCenter = function (str) {
        if (!str)
            return false;
        return this.clickSelectorCenter(textStartsWith(str));
    };
    ...
    this.launchActivity = function (activityName) {
        shell("am start -n " + this.packageName + "/" + activityName);
        waitForActivity(activityName);
    };
    this.kill = function () {
        shell("am force-stop " + this.packageName);
    };
    ...
    this.checkFile=function(dirPath){
